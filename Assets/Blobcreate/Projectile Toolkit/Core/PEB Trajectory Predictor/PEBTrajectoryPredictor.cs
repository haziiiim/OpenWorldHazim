...
namespace Blobcreate.ProjectileToolkit
{
	public class PEBTrajectoryPredictor : MonoBehaviour
	{
		...
		[Header("Aerodynamic Movement")]
		[Tooltip("Controls whether to predict aerodynamic movement. Note that the AMSolver property " +
		...
		/// <summary>
		/// Controls whether to predict aerodynamic movement. Note that the AMSolver property
		/// needs to be assigned for this to take effect.
		/// </summary>
		...
		/// <summary>
		/// The AerodynamicMoveSolver instance that will be used to predict aerodynamic movement.
		/// </summary>
		public AerodynamicMoveSolver AMSolver { get; set; }
		...
		void SimulateTrajectory(Vector3 velocity)
		{
				...
				i++)
			{
				if (i > 0)
				{
					if (predictAeroMove && AMSolver != null)
						AMSolver.ApplyAcceleration(ghostRigid, ref amTimer);
